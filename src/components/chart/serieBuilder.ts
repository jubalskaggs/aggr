...
class SerieBuilder {
   ...
   */
  build(indicators: { [id: string]: IndicatorSettings }, indicatorId: string) {
    const indicator = indicators[indicatorId]
    const otherIndicatorsSeriesIds = Object.keys(indicators)
      .filter(otherIndicatorId => otherIndicatorId !== indicatorId)
      .reduce((acc, otherIndicatorId) => {
        if (!indicators[otherIndicatorId].model) {
          return acc
        }

        return Object.keys(indicators[otherIndicatorId].model.plots)
      }, [])
    ...
    return indicators[indicatorId].model
  }
  ...
  parseReferences(output: string, references: IndicatorReference[]): string {
  ...
  buildSequence(indicators) {
    ...
    for (const indicatorId in indicators) {
      delete indicators[indicatorId].model
    }
